# Lithuanian Interface Review Reference

This file is a single-place reference for Lithuanian interface copy added/updated by the Full Lithuanian Interface feature.
Updated per native speaker review (jbagd, 2026-04-15).

## Navigation and Header
- Lietuviškai
- Praktika
- Moduliai
- Skaičiai 1-20
- Skaičiai 1-99
- Amžius
- Oras
- Kainos
- Laikas
- Bendra praktika
- Apie
- Statistika
- Atsiliepimai
- Atsijungti
- Prisijungti
- English | Lietuviškai

## Login and Landing
- Kainos lietuviškai - testas
- Kainos lietuviškai - praktika
- Prisijungti su Google
- Nemokama ir privatu. Saugome tik testų duomenis jūsų privačiam naudojimui.
- Lietuvių kalbos praktika
- Mokykitės kalbėti lietuviškai per adaptyvias užduotis
- Pradėkite praktiką
- Pradėkite čia
- Nemokama. Sekimas nevykdomas už dabartinės naršyklės sesijos ribų.
- Prisijunkite
- tik jei norite išsaugoti pažangą tarp apsilankymų.

## Error and Utility Pages
- Įvyko klaida
- Prisijungimas nepavyko. Taip dažniausiai nutinka atšaukus Google prisijungimą.
- Bandyti dar kartą
- Atgal į pradžią
- Puslapis nerastas
- Jūsų ieškomas puslapis neegzistuoja.

## Common Controls
- Atstatyti pažangą?
- Atšaukti
- Atstatyti
- Atstatyti pažangą
- Rodyti pavyzdį
- Pateikti
- Dabartinė užduotis
- Įrašykite atsakymą lietuviškai...

## Module Titles and Descriptions
- Kalbėjimo apie kainas praktika
- Mokykitės kalbėti apie kainas. Įrašykite pilną atsakymą su žodžiu euras.
- Du užduočių tipai:
- Kokia kaina? (vardininkas) ir Kiek kainuoja? (galininkas).

- Kalbėjimo apie laiką praktika
- Mokykitės kalbėti apie laiką. Įrašykite pilną atsakymą.
- Keturi užduočių tipai: pilnos valandos, pusė, penkiolika minučių po, be penkiolikos minučių.

- Kalbėjimo apie amžių praktika
- Mokykitės kalbėti apie amžių parinkdami tinkamą įvardį ir linksnį.
- Kūrimas (pasakykite amžių lietuviškai) ir atpažinimas (atpažinkite amžių iš lietuviškos frazės).

- Kalbėjimas apie orą
- Kalbėjimo apie temperatūrą praktika.
- Kūrimas (pasakykite temperatūrą lietuviškai) ir atpažinimas (atpažinkite temperatūrą iš lietuviškos frazės).

- Skaičiai 1-20
- Mokykitės kalbėti apie skaičius.

- Skaičiai 1-99
- Visi skaičiai, įskaitant dešimtis ir sudėtinius.

- Bendra praktika
- Atsitiktinės užduotys iš visų modulių, daugiau dėmesio silpniausioms vietoms.

## Question Prompt Templates (Dynamic)
- Kaip pasakyti {number}?
- Koks skaičius yra {number_word}?
- {dative_pronoun} yra {number} metų.
- Kaip pasakyti {signed_number}°C?

## Feedback and Grammar UI
- Teisingai!
- Jūsų atsakymas:
- Netikslu
- Teisingas atsakymas:
- Gramatikos paaiškinimas
- Vardininkas
- Galininkas
- Kilmininkas
- Vardininkas: vartojamas nurodant kainą (Kokia kaina?).
- Galininkas: vartojamas sakant, kiek kas kainuoja (Kiek kainuoja?).
- Kilmininkas: vartojamas su pusė/ketvirtis.

## Stats UI
- Jūsų statistika
- Sekite savo mokymosi pažangą
- Iš viso
- Teisingi
- Neteisingi
- Serija
- Tikslumas
- Tobulintinos sritys
- Skirkite dėmesį šioms sritims
- Atlikite daugiau užduočių
- Istorija
- Jūsų užduotys
- Istorijos dar nėra
- Čia bus rodoma jūsų užduočių istorija
- Žiūrėti viską
- Peržiūrėkite ankstesnes užduotis

---

## Added for 0-99 consolidation (2026-04-17)

**DB row for `number=0`** — please spot-check these forms:

- `nulis` (nominative; "Kaip pasakyti 0?" → "nulis")
- `nulį` (accusative; price answer for €0, not user-facing because prices filter to n≥1)
- `nulis eurų` (zero euros; genitive plural; prices completeness only)
- `nulis laipsnių` (zero degrees; used in weather produce answers)

## About Page
- Apie šią programą
- Mokykitės kalbėti apie skaičius su adaptyviomis užduotimis!
- Ši programa skirta mokytis kalbėti apie skaičius, kainas ir laiką naudojant interaktyvias užduotis.
- Užduotys apie skaičius
- Du moduliai skaičių žodynui plėsti:
- Užduotys apie amžių
- Užduotys apie orą
- Užduotys apie kainas
- Užduotys apie laiką
- Neigiamos temperatūros (iki -20) prideda minus prieš skaičiaus žodį.
- Gerinkite lietuvių kalbos įgūdžius per reguliarią praktiką!
- Sukūrė
- Atgal į praktiką
